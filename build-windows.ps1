...
if (Get-Command nmake -ErrorAction SilentlyContinue) {
    ...
    Write-Host ""
    Write-Host "WARNUNG: Sie befinden sich nicht in einer Visual Studio Developer-Umgebung!" -ForegroundColor Yellow
    Write-Host ""
    Write-Host "Bitte oeffnen Sie:" -ForegroundColor Yellow
    Write-Host "  Start -> Visual Studio 2022 -> Developer Command Prompt for VS 2022" -ForegroundColor White
    Write-Host "  Dann: powershell" -ForegroundColor White
    Write-Host ""
    Write-Host "Oder verwenden Sie:" -ForegroundColor Yellow
    Write-Host "  .\Import-VsDevShell.ps1" -ForegroundColor White
    Write-Host ""
    exit 1
}
...
if ($QtPath -and (Test-Path "$QtPath\bin\qmake.exe")) {
    ...
    foreach ($path in $qtPaths) {
        if (Test-Path "$path\bin\qmake.exe") {
            $qt6ConfigPath = "$path\lib\cmake\Qt6\Qt6Config.cmake"
            if (Test-Path $qt6ConfigPath) {
                $foundQt = $path
                Write-Host "OK Qt gefunden (automatisch): $foundQt" -ForegroundColor Green
                break
            } else {
                Write-Host "WARNUNG: Qt in $path gefunden, aber Qt6Config.cmake fehlt" -ForegroundColor Yellow
            }
        }
    }
}
...
Write-Host "   Generator: NMake Makefiles" -ForegroundColor Cyan
