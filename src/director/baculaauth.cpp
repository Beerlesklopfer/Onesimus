...
{
    BLOG_DEBUG() << "========================================";
    BLOG_DEBUG() << "BACULA AUTHENTICATION START";
    BLOG_DEBUG() << "========================================";
    BLOG_DEBUG() << "Director:" << directorName;
    BLOG_DEBUG() << "Console:" << consoleName;
    BLOG_DEBUG() << "Password length:" << password.length();
    BLOG_DEBUG() << "TLS Enable:" << tlsEnable;
    BLOG_DEBUG() << "TLS Require:" << tlsRequire;
    BLOG_DEBUG() << "PSK Enable:" << tlsPSKEnable;

    // Check for [md5] prefix (Bareos/Bacula config file format)
    // Format: [md5]01234abcdef... where the hash follows the prefix
    if (password.startsWith("[md5]", Qt::CaseInsensitive)) {
        m_password = password.mid(5);  // Remove "[md5]" prefix
        BLOG_DEBUG() << "Detected [md5] prefix, stripped password";
    } else {
        m_password = password;
    }
    m_authSuccess = false;
    ...
    BLOG_DEBUG() << "TLSPSK local need:" << tlspskLocalNeed;
    BLOG_DEBUG() << "  TLS local need:" << m_tlsLocalNeed;
    BLOG_DEBUG() << "  PSK local need:" << m_pskLocalNeed;
    ...
    BLOG_DEBUG() << "<<< Director response:" << response;
    ...
    if (match.hasMatch()) {
        ...
        BLOG_DEBUG() << "\n=== STARTTLS RECEIVED ===";
        BLOG_DEBUG() << "Remote TLSPSK:" << tlspskRemote;
        ...
        BLOG_DEBUG() << "<<< Post-TLS response:" << response;
    }
    ...
    BLOG_DEBUG() << "<<< Auth response:" << authResponse;
    ...
    BLOG_DEBUG() << "<<< Final response:" << finalResponse;
    ...
    BLOG_DEBUG() << "  Director Version:" << m_directorVersion;
    BLOG_DEBUG() << "========================================";
...
void BaculaAuth::decodeRemoteTLSPSKNeed(int remoteNeed)
{
    ...
    BLOG_DEBUG() << "Remote TLSPSK decoded:" << remoteNeed;
    BLOG_DEBUG() << "  TLS Remote Need:" << m_tlsRemoteNeed;
    BLOG_DEBUG() << "  PSK Remote Need:" << m_pskRemoteNeed;
}
...
bool BaculaAuth::sendBaculaMessage(const QString &message)
{
    ...
    BLOG_DEBUG() << ">>> Sending Bacula packet:";
    BLOG_DEBUG() << "    Length header:" << QString("0x%1").arg(message.toUtf8().size(), 8, 16, QChar('0'));
    BLOG_DEBUG() << "    Message:" << message.trimmed();
    BLOG_DEBUG() << "    Total packet size:" << packet.size() << "bytes";
...
bool BaculaAuth::readBaculaMessage(QString &message, int timeoutMs)
{
    ...
    while (m_readBuffer.size() < 4) {
        if (!waitForResponse(timeoutMs)) {
            BLOG_DEBUG() << "Timeout waiting for packet header";
            return false;
        }
    ...
    BLOG_DEBUG() << "  Packet header: length =" << length;

    // Negative Länge = Signal-Nachricht (z.B. -1 für EOD)
    if (length < 0) {
        BLOG_DEBUG() << "  Signal message received:" << length;
    ...
    while (m_readBuffer.size() < 4 + length) {
        if (!waitForResponse(timeoutMs)) {
            BLOG_DEBUG() << "Timeout waiting for packet data";
            return false;
        }
        m_readBuffer.append(m_socket->readAll());
    }
    ...
    BLOG_DEBUG() << "<<< Read Bacula message:" << message.left(100)
             << (message.length() > 100 ? "..." : "");
...
{
    BLOG_DEBUG() << "--- CRAM-MD5 Respond Phase ---";
    ...
    BLOG_DEBUG() << "<<< Received challenge:" << challenge;
    ...
    if (match.hasMatch()) {
        ...
        // Extract real Director name from challenge: <random.timestamp@DirectorName>
        int atIdx = challengeStr.indexOf('@');
        if (atIdx >= 0) {
            m_remoteDirectorName = challengeStr.mid(atIdx + 1);
            if (m_remoteDirectorName.endsWith('>'))
                m_remoteDirectorName.chop(1);
            BLOG_DEBUG() << "  Remote Director name:" << m_remoteDirectorName;
        }

        BLOG_DEBUG() << "  Challenge string:" << challengeStr;
        BLOG_DEBUG() << "  Remote SSL value:" << remoteSslValue;
        ...
        BLOG_DEBUG() << "  Stored remote TLS need:" << m_tlsRemoteNeed;
    ...
    BLOG_DEBUG() << ">>> Sending response:";
    BLOG_DEBUG() << "    Base64: " << responseBase64;
    BLOG_DEBUG() << "    Hex:    " << hmac.toHex();
    BLOG_DEBUG() << "    (Password length:" << password.length() << "chars)";
    ...
    BLOG_DEBUG() << "✓ CRAM-MD5 response sent successfully";
...
{
    BLOG_DEBUG() << "--- CRAM-MD5 Challenge Phase ---";
    ...
    BLOG_DEBUG() << "Generated challenge:" << challenge;
    ...
    BLOG_DEBUG() << "  TLS remote need:" << m_tlsRemoteNeed;
    BLOG_DEBUG() << "  PSK remote need:" << m_pskRemoteNeed;
    BLOG_DEBUG() << "  SSL value to send:" << sslValue;
    ...
    BLOG_DEBUG() << ">>> Sending challenge:" << challengeMsg.trimmed();
    ...
    BLOG_DEBUG() << "<<< Received Director response:" << dirResponse;
    ...
    BLOG_DEBUG() << "  Expected response (Base64):" << expectedBase64;
    BLOG_DEBUG() << "  Expected response (Hex):" << expectedHmac.toHex();
    BLOG_DEBUG() << "  Received response:" << dirResponse.trimmed();
    ...
    BLOG_DEBUG() << "✓ Director's CRAM-MD5 response verified successfully";
    return true;
}
...
{
    BLOG_DEBUG() << "========================================";
    BLOG_DEBUG() << "STARTING" << (usePSK ? "PSK-TLS" : "TLS") << "ENCRYPTION";
    BLOG_DEBUG() << "========================================";
    BLOG_DEBUG() << "Password available:" << !m_password.isEmpty();
    BLOG_DEBUG() << "Password length:" << m_password.length();
    ...
    if (usePSK) {
        ...
        QObject::connect(m_socket, &QSslSocket::preSharedKeyAuthenticationRequired,
                         this, [password](QSslPreSharedKeyAuthenticator *authenticator) {
                             ...
                             BLOG_DEBUG() << "========================================";
                             BLOG_DEBUG() << "!!! PSK CALLBACK TRIGGERED !!!";
                             BLOG_DEBUG() << "========================================";
                             BLOG_DEBUG() << "  Identity hint:" << authenticator->identityHint();
                             BLOG_DEBUG() << "  Max identity length:" << authenticator->maximumIdentityLength();
                             BLOG_DEBUG() << "  Max PSK length:" << authenticator->maximumPreSharedKeyLength();
                             BLOG_DEBUG() << "  Password length:" << password.length();

                             if (password.isEmpty()) {
                                 BLOG_ERROR() << "  ERROR: Password is empty!";
                                 return;
                             }
                             ...
                             BLOG_DEBUG() << "  ✓ PSK credentials set!";
                             BLOG_DEBUG() << "  Identity: (empty)";
                             BLOG_DEBUG() << "  PSK: [" << psk.length() << " bytes]";
                             BLOG_DEBUG() << "========================================";

                         }, Qt::DirectConnection);  // DirectConnection ist wichtig!

        BLOG_DEBUG() << "✓ PSK callback registered";
        ...
        for (const QSslCipher &cipher : QSslConfiguration::supportedCiphers()) {
            ...
            if (cipher.name() == "PSK-AES256-CBC-SHA") {
                ...
                BLOG_DEBUG() << "  Using Bacula PSK cipher:" << cipher.name();
        ...
        if (pskCiphers.isEmpty()) {
            ...
            for (const QSslCipher &cipher : QSslConfiguration::supportedCiphers()) {
                if (cipher.name().startsWith("PSK-")) {
                    pskCiphers.append(cipher);
                    BLOG_DEBUG() << "  Fallback PSK cipher:" << cipher.name();
                }
            }
        }
        ...
        if (pskCiphers.isEmpty()) {
            ...
            BLOG_ERROR() << m_errorMessage;
            BLOG_ERROR() << "Your OpenSSL/Qt may not support PSK ciphers.";
            return false;
        }

        sslConfig.setCiphers(pskCiphers);
        BLOG_DEBUG() << "PSK ciphers configured:" << pskCiphers.size();
        ...
        BLOG_DEBUG() << "Using standard TLS configuration";
    }

    // Konfiguration anwenden
    m_socket->setSslConfiguration(sslConfig);

    BLOG_DEBUG() << "Starting client encryption...";
    ...
    if (!m_socket->waitForEncrypted(10000)) {
        ...
        // Debug-Info
        // BLOG_DEBUG() << "SSL Errors:";
        // for (const QSslError &error : m_socket->sslErrors() ) {
        //     BLOG_DEBUG() << "  -" << error.errorString();
        // }
    ...
    BLOG_DEBUG() << "========================================";
    BLOG_DEBUG() << "✓" << (usePSK ? "PSK-TLS" : "TLS") << "ENCRYPTION ESTABLISHED";
    BLOG_DEBUG() << "  Protocol:" << m_socket->sessionProtocol();
    BLOG_DEBUG() << "  Cipher:" << m_socket->sessionCipher().name();
    BLOG_DEBUG() << "  Encrypted:" << m_socket->isEncrypted();
    BLOG_DEBUG() << "========================================";
...
bool BaculaAuth::clientEarlyTLS()
{
    ...
    if (match.hasMatch()) {
        ...
        BLOG_DEBUG() << "Director sent CRAM-MD5 challenge directly, no TLS negotiation";
...
bool BaculaAuth::handleTLS()
{
    ...
    if (m_tlsLocalNeed >= BNET_TLS_OK && m_tlsRemoteNeed >= BNET_TLS_OK) {
        ...
        if (!m_socket->isEncrypted()) {
            ...
            BLOG_DEBUG() << "✓ PSK-TLS encryption established";
#else
            BLOG_WARNING() << "⚠ PSK-TLS not supported in Qt < 5.12, using plain connection";
            BLOG_WARNING() << "   Upgrade to Qt 5.12+ for proper PSK support";
#endif
        } else {
            BLOG_DEBUG() << "✓ Socket already encrypted (PSK)";
        }

        m_tlsStarted = true;

    } else {
        BLOG_DEBUG() << "Using clear text connection (no encryption)";
        m_tlsStarted = false;
    }

    BLOG_DEBUG() << "TLS/PSK handling complete. Encrypted:" << m_socket->isEncrypted();
    BLOG_DEBUG() << "========================================";
    return true;
}
...
bool BaculaAuth::clientCramMD5Authenticate(const QString &password)
{
    ...
    BLOG_DEBUG() << "========================================";
    BLOG_DEBUG() << "CRAM-MD5 AUTHENTICATION";
    BLOG_DEBUG() << "========================================";
    ...
    BLOG_DEBUG() << "✓ CRAM-MD5 authentication successful";
    BLOG_DEBUG() << "========================================";
    return true;
}
...
void BaculaAuth::startAuthTimeout()
{
    ...
    BLOG_DEBUG() << "⏱ Authentication timeout started:" << AUTH_TIMEOUT << "ms";
}
...
void BaculaAuth::stopAuthTimeout()
{
    if (m_authTimer->isActive()) {
        m_authTimer->stop();
        BLOG_DEBUG() << "⏱ Authentication timeout stopped";
    }
}
...
{
    BLOG_ERROR() << "========================================";
    BLOG_ERROR() << "⏱ AUTHENTICATION TIMEOUT!";
    BLOG_ERROR() << "========================================";
