#include "models/bdirectormodel.h"
                                    ...
                                    const QString &backupSystem)
{
    ...
    if (!BPasswordUtil::isValidPasswordHash(passwordHash)) {
        BLOG_WARNING() << "Invalid password hash format";
        return -1;
    }
    ...
    // Submit to database
    if (!submitAll()) {
        BLOG_WARNING() << "Failed to create director:" << lastError().text();
        revertAll();
        return -1;
    }
...
bool BDirectorModel::deleteDirector(int directorId)
{
    ...
    if (success) {
        ...
        BSettingsHistory::deleteAllSettings(db, "director", directorId);
