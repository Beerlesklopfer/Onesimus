...
{
    Q_UNUSED(backupSystem)  // TODO: Store backup system in database if needed
    ...
    if (!query.exec()) {
        BLOG_WARNING() << "Failed to set setting:" << query.lastError().text();
        return false;
    }
                                                         ...
                                                         int resourceId)
{
    ...
    if (query.exec()) {
        ...
        BLOG_WARNING() << "Failed to get all settings:" << query.lastError().text();
    }
                                                          ...
                                                          const QString &key)
{
    ...
    if (query.exec()) {
        ...
        BLOG_WARNING() << "Failed to get history:" << query.lastError().text();
    }
                                                             ...
                                                             int resourceId)
{
    ...
    if (query.exec()) {
        ...
        BLOG_WARNING() << "Failed to get all history:" << query.lastError().text();
    }
                                        ...
                                        const QString &reason)
{
    ...
    if (history.size() < 2) {
        BLOG_WARNING() << "Cannot rollback: no previous value exists";
        return false;
    }
                                      ...
                                      const QString &reason)
{
    ...
    if (!query.exec()) {
        BLOG_WARNING() << "Failed to delete setting:" << query.lastError().text();
        return false;
    }
                                          ...
                                          int resourceId)
{
    ...
    if (!query.exec()) {
        BLOG_WARNING() << "Failed to delete all settings:" << query.lastError().text();
        return false;
    }
                                                                ...
                                                                int limit)
{
    ...
    if (query.exec()) {
        ...
        BLOG_WARNING() << "Failed to get changes by user:" << query.lastError().text();
    }
...
QList<BSettingHistoryEntry> BSettingsHistory::getRecentChanges(QSqlDatabase &db, int limit)
{
    ...
    if (query.exec()) {
        ...
        BLOG_WARNING() << "Failed to get recent changes:" << query.lastError().text();
    }
